...
public class FragmentMessages extends FragmentEx {
        ...
        new ItemTouchHelper(
                        new ItemTouchHelper.Callback() {
                            ...
                            public void onSwiped(
                                ...
                                Log.i(
                                        Helper.TAG,
                                        "Swiped dir=" + direction + " message=" + message.id);

                                Bundle args = new Bundle();
                                args.putLong("id", message.id);
                                args.putBoolean(
                                        "thread", viewType != AdapterMessage.ViewType.THREAD);
                                ...
                                new SimpleTask<MessageTarget>() {
                                    ...
                                    protected MessageTarget onLoad(Context context, Bundle args) {
                                        ...
                                        try {
                                            ...
                                            EntityFolder folder =
                                                    db.folder().getFolder(message.folder);
                                    ...
                                    protected void onLoaded(
                                            final Bundle args, final MessageTarget result) {
                                        // Show undo snackbar
        ...
        bottom_navigation.setOnNavigationItemSelectedListener(
                new BottomNavigationView.OnNavigationItemSelectedListener() {
                    ...
                    public boolean onNavigationItemSelected(@NonNull MenuItem menuItem) {
                        ViewModelMessages.Target[] pn =
                                (ViewModelMessages.Target[]) bottom_navigation.getTag();
                                ...
                                Collections.sort(
                                        ...
                                        new Comparator<EntityFolder>() {
                                            ...
                                            public int compare(EntityFolder f1, EntityFolder f2) {
                                                int s =
                                                        Integer.compare(
                                                                EntityFolder.FOLDER_SORT_ORDER
                                                                        .indexOf(f1.type),
                                                                EntityFolder.FOLDER_SORT_ORDER
                                                                        .indexOf(f2.type));
                                                if (s != 0) {
                            ...
                            protected void onLoaded(final Bundle args, List<EntityFolder> folders) {
                                ...
                                for (EntityFolder folder : folders) {
                                    String name =
                                            (folder.display == null
                                                    ? Helper.localizeFolderName(
                                                            getContext(), folder.name)
                                                    : folder.display);
                                    popupMenu
                                            .getMenu()
                                            .add(Menu.NONE, folder.id.intValue(), order++, name);
                                }
                                ...
                                popupMenu.setOnMenuItemClickListener(
                                        new PopupMenu.OnMenuItemClickListener() {
                                            ...
                                            public boolean onMenuItemClick(final MenuItem target) {
                                                MutableSelection<Long> selection =
                                                                ...
                                                                List<EntityMessage> messages =
                                                                        db.message()
                                                                                .getMessageByThread(
                                                                                        message.account,
                                                                                        message.thread);
                                                                for (EntityMessage threaded :
                                                                        messages) {
                                                                    if (threaded.folder.equals(
                                                                            message.folder)) {
                                                                        db.message()
                                                                                .setMessageUiHide(
                                                                                        threaded.id,
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        grpHintSupport.setVisibility(
                prefs.getBoolean("app_support", false)
                                || viewType != AdapterMessage.ViewType.UNIFIED
                        ? View.GONE
                        : View.VISIBLE);
        grpHintSwipe.setVisibility(
                prefs.getBoolean("message_swipe", false)
                                || viewType == AdapterMessage.ViewType.THREAD
                        ? View.GONE
                        : View.VISIBLE);
        grpHintSelect.setVisibility(
                prefs.getBoolean("message_select", false)
                                || viewType != AdapterMessage.ViewType.FOLDER
                        ? View.GONE
                        : View.VISIBLE);
        ...
        if (viewType == AdapterMessage.ViewType.THREAD) {
            ...
            bottom_navigation.setVisibility(
                    pn[0] == null && pn[1] == null ? View.GONE : View.VISIBLE);
        } else {
            // Compose FAB
            Bundle args = new Bundle();
        ...
        switch (item.getItemId()) {
            ...
            case R.id.menu_sort_on_starred:
                prefs.edit()
                        .putString(
                                "sort",
                                item.getItemId() == R.id.menu_sort_on_unread ? "unread" : "starred")
                        .apply();
                item.setChecked(true);
                loadMessages();
                return true;
            ...
            protected Void onLoad(Context context, Bundle args) throws Throwable {
                ...
                try {
                    ...
                    for (EntityMessage message : db.message().getMessageSeen(outbox)) {
                        ...
                        EntityFolder sent =
                                db.folder().getFolderByType(identity.account, EntityFolder.SENT);
                        if (sent != null) {
    ...
    private void loadMessages() {
        ...
        if (TextUtils.isEmpty(search)) {
                ...
                case UNIFIED:
                    messages =
                            new LivePagedListBuilder<>(
                                            db.message().pagedUnifiedInbox(sort, debug),
                                            LOCAL_PAGE_SIZE)
                                    .build();
                    break;
                case FOLDER:
                    ...
                    LivePagedListBuilder<Integer, TupleMessageEx> builder =
                            new LivePagedListBuilder<>(
                                    db.message()
                                            .pagedFolder(folder, folderType, sort, false, debug),
                                    config);
                    if (browse) {
                ...
                case THREAD:
                    messages =
                            new LivePagedListBuilder<>(
                                            db.message()
                                                    .pagedThread(
                                                            account, folder, thread, sort, debug),
                                            LOCAL_PAGE_SIZE)
                                    .build();
                    break;
            }
            ...
            LivePagedListBuilder<Integer, TupleMessageEx> builder =
                    new LivePagedListBuilder<>(
                            db.message().pagedFolder(folder, folderType, "time", true, false),
                            config);
            builder.setBoundaryCallback(searchCallback);
            messages = builder.build();
        }
        ...
        messages.observe(
                ...
                new Observer<PagedList<TupleMessageEx>>() {
                    ...
                    public void onChanged(@Nullable PagedList<TupleMessageEx> messages) {
                        ...
                        boolean searching =
                                (searchCallback != null && searchCallback.isSearching());
